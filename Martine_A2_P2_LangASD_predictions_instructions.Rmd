...
```{r}
#Comparing basic model with our model 
...
ggplot(our_vs_simple)+
  geom_point(aes(x=folds, y = diff_simple ))+
  geom_smooth(aes(x=folds, y = diff_simple),method = "lm", alpha = 0)+
  geom_point(aes(x=folds, y= diff_our))+
  geom_smooth(aes(x=folds, y = diff_our),method = "lm", alpha= 0)
    
  
t.test(diff_our ~ diff_simple, our_vs_simple)
?t.test
```
