...
for (fold in folds){
  ...
  test_m1 = subset(train_data, Child.ID %in% fold)
  model_m1 = lmer(CHI_MLU ~ I(Visit^2) + Visit*Diagnosis + (1+I(Visit^2)|Child.ID), train_m1, REML = FALSE)
  model_m2 = lmer(CHI_MLU ~Visit*Diagnosis + (1+Visit|Child.ID), train_m1, REML = FALSE)
  test_m1$prediction = predict(model_m1, test_m1, allow.new.levels = TRUE)
  train_m1$prediction = fitted(model_m1)
  test_m1$prediction_m2 = predict(model_m2, test_m1, allow.new.levels = TRUE)
  train_m1$prediction_m2 = fitted(model_m2)
  trainRMSE_m1[i] = RMSE(train_m1$CHI_MLU, fitted(model_m1))
  testRMSE_m1[i] = RMSE(test_m1$CHI_MLU, test_m1$prediction)
  trainRMSE_m2[i] = RMSE(train_m1$CHI_MLU, fitted(model_m2))
  testRMSE_m2[i] = RMSE(test_m1$CHI_MLU, test_m1$prediction_m2)
  i = i + 1
}

RMSE_m1 <- data.frame(trainRMSE_m1, testRMSE_m1, trainRMSE_m2, testRMSE_m2)

# Cross validation function
cross_valid <- function(data, k, model){
  folds = createFolds(unique(train_data$Child.ID), k = k, list = TRUE, returnTrain = FALSE)
...
for (fold in folds){
  ...
  model = lmer(model, train, REML = FALSE)
...
#testing it
mm1 <-  CHI_MLU ~ I(Visit^2) + Visit*Diagnosis + (1+I(Visit^2)|Child.ID)
cv_test <- cross_valid(train_data, 6, mm1)
cv_test
