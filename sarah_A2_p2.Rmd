...
pacman::p_load(readr,dplyr,stringr,lmerTest,Metrics,caret, tidyverse, MuMIn)

train_data <- read.csv('data_clean.csv')

Demo <- read.csv('demo_test.csv')
LU <- read.csv('LU_test.csv')
Word <- read.csv('token_test.csv')
...
growth <- lmer(CHI_MLU ~ Visit*Diagnosis+
                           I(Visit^2)+(1+I(Visit^2)|Child.ID), train_data, REML = FALSE)

m <- lmer(CHI_MLU ~ I(Visit^2)+ Visit*Diagnosis + (1+I(Visit^2)|Child.ID), train_data, REML=F)

max <- lmer(CHI_MLU ~ I(Visit^2)+Visit*Diagnosis+ Socialization*Diagnosis + verbalIQ1+(1|Child.ID), train_data, REML = F)
...
rmse(df$CHI_MLU, predict(max))
rmse(df$CHI_MLU, predict(growth))
...
folds = createFolds(unique(Data$Child.ID), k = k, list = TRUE, returnTrain = FALSE)
k = 6
...
fitControl <- trainControl(method = "repeatedcv",   
                           ...
                           repeats = 6)    # repeated ten times

train_data <- na.omit(train_data)
model.cv <- train(CHI_MLU ~ ., data = train_data, trControl = fitControl)  
model.cv
...
```{r}
b <- predict(max, newdata = Bernie, allow.new.levels = T)
