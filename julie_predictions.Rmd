...
k = 6
folds = createFolds(unique(df$SUBJ, k = k, list = TRUE, returnTrain = FALSE)
...
for (fold in folds){
  ...
  model = lmer(CHI_MLU ~ VISIT*Diagnosis+(1|SUBJ), train)
...
rmse(df_test$CHI_MLU, fitted(max))
predict(max, newdata = df_test, allow.new.levels = TRUE)
...
k = 6
folds = createFolds(unique(data_test$Child.ID, k = k, list = TRUE, returnTrain = FALSE)
...
i = 1
for (fold in folds){
  train = subset(train_df, !(SUBJ %in% fold))
  test = subset(train_df, SUBJ %in% fold)
  model = lmer(CHI_MLU ~ I(VISIT^2)+VISIT*Diagnosis+ Socialization*Diagnosis + verbalIQ1+(1|SUBJ), train, REML = F)
  test$prediction = predict(model, test, allow.new.levels = TRUE)
  train$prediction = fitted(model)
  trainRMSE[i] = rmse(train$CHI_MLU, fitted(model))
  testRMSE[i] = rmse(test$CHI_MLU, test$prediction)
...
```{r}
hey <- lm(types_CHI~Diagnosis*Visit, data = data_test)
summary(hey)
hey1 <- lm(types_CHI~Diagnosis+Visit, data = data_test)
summary(hey1)
```
