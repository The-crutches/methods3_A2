...
# Load training Data
df <- read_csv("~/Uni stuff/Expmeth3/methods3_A2_P1/autism_df.csv")

#- recreate the models you chose last time (just write the code again and apply it to Train Data)
train_chi <- lmerTest::lmer(CHI_MLU ~ VISIT*Diagnosis+I(VISIT^2)+(1+I(VISIT^2)|SUBJ),
                      df,
                     REML = FALSE)
...
data_test <- CleanUpData(demo, LU, token)
...
i = 1
for (fold in folds){
  train = subset(data_test, !(Child.ID %in% fold))
  test = subset(data_test, Child.ID %in% fold)
  model = lmer(CHI_MLU ~ Visit*Diagnosis+(1|Child.ID), train)
