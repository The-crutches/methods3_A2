...
#Loading packages
```{r}
pacman::p_load(readr,dplyr,stringr,lmerTest,Metrics,caret, data.table)
```
...
diff_complex <- trainRMSE_our - testRMSE_our
  #Making difference columns for simple model
diff_simple <- trainRMSE_simple - testRMSE_simple
  
#Dataframe
our_vs_simple <- data.frame(trainRMSE_our, testRMSE_our, diff_complex, trainRMSE_simple, testRMSE_simple, diff_simple)
  #Fold column
our_vs_simple$folds <- c(1:6)

#Plotting our model against the simple model
our_vs_simple$folds <- as.factor(our_vs_simple$folds)
our_vs_simple <- select(our_vs_simple, diff_simple, diff_complex, folds) #selecting relevant data
diff_df <- gather(our_vs_simple, difference, value, diff_complex, diff_simple, factor_key = T) #changing data format to long
...
test_3 <- cross_valid(train_df, 6, bmaxv)
