...
rmse(data_clean$CHI_MLU, fitted(train_chi))
rmse(df$CHI_MLU, fitted(bmaxv))

##What is a good rmse-value: A value near 1.0 indicates an almost perfect fit while values near zero result from a model where the predictions have no linear association with the outcome. One other advantage of this number is that it makes comparisons between different outcomes easy since it is unitless.
...
predict(bmaxv, newdata = df_test, allow.new.levels = TRUE)


rmse(df_test$CHI_MLU, predict(bmaxv))
