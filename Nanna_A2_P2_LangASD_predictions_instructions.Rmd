...
#Plotting Bernie against means for TD and ASD
ggplot(Bernie) +
...
#NEdenstående er lavet af Julie og NAnananan
```{r}
...
#Noget andet
pred_max <- predict(bmaxv, newdata = df_test, allow.new.levels = TRUE)

df_test <- data.frame(df_test , pred_max)

df_test$compared <- (df_test$pred_max - df_test$CHI_MLU) 

ggplot(df_test, aes( x = pred_max , y = CHI_MLU )) + geom_point() + facet_wrap(~Child.ID)
